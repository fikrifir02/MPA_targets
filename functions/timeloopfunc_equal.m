...
function xres = timeloopfunc_equal(Aval_null, umat, nspp, Ninit,spparams, Ainit, tmax, startmpa,...
     nmonths, dT)
...
for t  =  2:tmax
    for ispp =  1:nspp
        ...
        fish_area_prop = A(1,ispp)+ A(3,ispp);% remaining fishing area proportion
        displaced_umat = umat{1,ispp}(:,2)*(A(2,ispp)/fish_area_prop);
        umat{1,ispp}(:,1) = umat{1,ispp}(:,1) + displaced_umat;
        umat{1,ispp}(:,2) = 0;
        umat{1,ispp}(:,3) = umat{1,ispp}(:,1);% same as fishing mortaility rate in NMPA
        %***

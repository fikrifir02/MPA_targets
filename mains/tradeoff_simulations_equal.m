...
flenm = char('../output/trade_off_result_PPA_equal.mat'); % output file named based on fishing effort reduction only when fishing effort inside FMPA equals to NMPA
...
for iu = 1:nuvals %simulate three different fishing pressures (FP)
	...
	xres_SQ1 = timeloopfunc_equal(SQ1_null, umat, output.nspp, output.Ninit,output.spparams, Ainit, tmax, startmpa,...
     output.nmonths, output.dT); % Simulate status quo condition over time
 
	Ninit = xres_SQ1.N1;
	tmax = 100;
    
	xres_SQ = timeloopfunc_equal(SQ_null, umat, output.nspp, Ninit,output.spparams, Ainit, tmax, startmpa,...
    output.nmonths, output.dT); % continue - Simulate status quo condition over time
    ...
    xres_main = timeloopfunc_equal([Avals(i,1);Avals(i,2)], umat, output.nspp, Ninit,output.spparams, Ainit, tmax, startmpa,...
     output.nmonths, output.dT);
        for j = 1: output.nspp
